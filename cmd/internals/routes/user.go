...
func UserRoutes(route *gin.Engine) {
	userRoutesGroup := route.Group("/users")
	{
		...
		userRoutesGroup.POST("/signin", func(c *gin.Context) {
			...
			if _, err := models.FindUserByUsername(curSigninUser.Username); err != nil {
			...
			err := scrypt.CompareHashAndPassword([]byte(curUser.Pass), []byte(curSigninUser.Password))
			if err != nil {
				c.JSON(http.StatusNotFound, gin.H{
					"error": "Login failed",
				})
